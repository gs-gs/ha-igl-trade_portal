...
@mock.patch("trade_portal.documents.services.notarize.NotaryService.notarize_file")
def test_document_service(notary_service_mock, oa_task_mock, docapi_env, ERROR_CODE):
    ...
    notary_service_mock.return_value = True if ERROR_CODE != "fail_notarize" else False
    ...
    if ERROR_CODE is None:
        ...
        assert notary_service_mock.call_count == 1
        notary_service_mock.assert_called_once_with(
        ...
        assert notary_service_mock.call_count == 0
        assert oa_client.wrap_document.call_count == 1
